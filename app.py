...
def render_settings_journal_groups(journals: list[dict], base_path: str) -> str:
    ...
    for publication_name, items in groups:
        group_key = slugify(publication_name) or "publication"
        rows = []
...
def render_settings_page(handler: BaseHTTPRequestHandler, query: dict, base_path: str) -> Optional[str]:
    ...
    body = (
        ...
        f'<section class="settings-card library-card"><div class="library-content"><div class="library-header"><div class="library-title"><span class="library-title-dot"></span><span>Bibliotheque</span></div><p class="library-subtitle">Une section repliable par publication. Suppression simple ou multiple.</p></div>{render_settings_journal_groups(journals, base_path)}</div></section>'
        "</main>"
        '<aside class="settings-sidebar">'
        '<section class="settings-card rules-card"><div class="rules-content"><div class="rules-header"><div class="rules-title"><span class="rules-title-dot"></span><span>Limite de fraicheur</span></div><p class="rules-subtitle">Bloque l\'ingestion au-dela de X jours.</p></div>'
        f'<form method="post" action="{escape_html(with_base_path(base_path, "/settings/retention"))}" class="rules-form"><input type="number" min="1" step="1" name="maxJournalAgeDays" class="rules-input" value="{escape_html(config["max_journal_age_days"] or "")}" placeholder="30" /><button type="submit" class="btn btn-add">Enregistrer</button></form></div></section>'
        '<section class="settings-card cleanup-card"><div class="cleanup-content"><div class="cleanup-header"><div class="cleanup-title"><span class="cleanup-title-dot"></span><span>Auto-destruction</span></div><p class="cleanup-subtitle">Supprime automatiquement les journaux plus vieux que X jours.</p></div>'
        f'<form method="post" action="{escape_html(with_base_path(base_path, "/settings/auto-delete"))}" class="cleanup-form"><input type="number" min="1" step="1" name="autoDeleteAfterDays" class="cleanup-input" value="{escape_html(config["auto_delete_after_days"] or "")}" placeholder="90" /><button type="submit" class="btn btn-add">Enregistrer</button></form></div></section>'
        '<section class="settings-card maintenance-card"><div class="maintenance-content"><div class="maintenance-header"><div class="maintenance-title"><span class="maintenance-title-dot"></span><span>Actions rapides</span></div><p class="maintenance-subtitle">Verification immediate ou fermeture de session.</p></div><div class="maintenance-actions">'
        f'<form method="post" action="{escape_html(with_base_path(base_path, "/settings/scan"))}"><button type="submit" class="btn btn-add">Lancer un scan immediat</button></form>'
        f'<form method="post" action="{escape_html(with_base_path(base_path, "/logout"))}"><button type="submit" class="btn btn-delete">Se deconnecter</button></form></div></div></section>'
        "</aside></div></section>"
    )
